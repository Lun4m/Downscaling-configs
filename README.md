1. Clone this repo in your home directory on LUMI
   ```term
   git clone git@github.com:Lun4m/Downscaling-configs.git
   cd Downscaling-configs
   ```

1. You need to be part of a project. LUMI projects are named `project_xxxxxxxxx`,
   where xxxxxxxxx is the number needed in the next step.

1. Add the following to your `~/.bashrc`, substituting xxxxxxxxx with your project number.
    ```bash
    # Project settings
...
1. Optional step: if you need to do development on Anemoi, install anemoi
   packages locally
    ```term
...
1. Build virtual environment
    ...
    > [!IMPORTANT] 
    > The built environments live in a shared directory on `$SCRATCH`.
    >
...
1. Authenticate to the MLFlow logging server
   ```term
